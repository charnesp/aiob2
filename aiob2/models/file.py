...
DownloadPayloadHeaders = TypedDict(
    "DownloadPayloadHeaders",
    {
        "Content-Length": int,
        "Content-Type": str,
        "X-Bz-File-Id": str,
...
class PartialFile(B2Object):
    ...
    def __init__(self, data: PartialFilePayload):
...
class File(PartialFile):
    ...
    def __init__(self, data: UploadPayload):
        super().__init__(data)
        ...
        self.replication_status: Optional[
            Literal["PENDING", "COMPLETED", "FAILED", "REPLICA"]
        ] = data.get(
            "replicationStatus"
        )  # noqa
...
class DownloadedFile(B2Object):
    ...
    def __init__(self, content: bytes, headers: DownloadPayloadHeaders):
        ...
        self.created: datetime.datetime = format_timestamp(
            int(headers["X-Bz-Upload-Timestamp"])
        )
