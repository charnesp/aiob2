...
async def json_or_bytes(
    response: aiohttp.ClientResponse, route: Route
) -> Union[Dict[str, Any], bytes]:
    if route.path == "/b2api/v2/b2_download_file_by_id" or route.path.startswith(
        "/file/"
    ):
...
class HTTPClient:
    ...
    async def _authorize_account(self) -> None:
        ...
        response: AccountAuthorizationPayload = await self.request(
    ...
    async def _get_upload_url(self, bucket_id: str) -> BucketUploadInfo:
        ...
        response: UploadURLPayload = await self.request(
            route, headers=headers, params=params
        )
    ...
    async def request(
        ...
        if (
            self._account_id is MISSING and route.path != "/b2_authorize_account"
        ):  # prevent recursion death loop
            ...
            async with self._session.request(
                ...
                if response.status == 401:
                    ...
                    if data["code"] in ("expired_auth_token", "bad_auth_token"):
                        ...
                        if route.action == "upload":
                            assert bucket_id is not None
                            self._upload_urls[bucket_id] = await self._get_upload_url(
                                bucket_id
                            )
    ...
    async def upload_file(
        ...
        return self.request(
            route, bucket_id=bucket_id, headers=headers, data=content_bytes
        )

    def delete_file(
        self, *, file_name: str, file_id: str
    ) -> Response[PartialFilePayload]:
        route = Route(
            "GET",
